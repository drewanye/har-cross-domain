...
def label_predictor(features, is_train, Y):
    with tf.variable_scope('label_predictor'):
        classifier_features = tf.cond(is_train, lambda: tf.slice(features, [0, 0], [batch_size, -1]),
                                     lambda: features)
        all_labels = Y
        labels = tf.cond(is_train, lambda: tf.slice(all_labels, [0, 0], [batch_size, -1]), lambda: all_labels)
        f_net1 = slim.fully_connected(classifier_features, 100, scope='full1')
        f_net2 = slim.fully_connected(f_net1, 100, scope='full2')
        # f_net3 = slim.fully_connected(f_net2, 64, scope='full3')
        # f_net4 = slim.fully_connected(f_net3, 64, scope='full4')
...
def domain_predictor(features, sigma):
    with tf.variable_scope('domain_predictor'):
        features = flip_gradient(features, sigma)
        # f_net1 = slim.fully_connected(features, 100, scope='full1')
...
def create_model(x, is_training, is_train, sigma, Y, prok):
    with tf.variable_scope('cnn_adversarial'):
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            with slim.arg_scope([slim.conv2d], kernel_size=[3, 3]):
                with slim.arg_scope([slim.max_pool2d], padding='SAME'):
                    ...
                    all_features = slim.flatten(all_features)

                    label_logits, labels = label_predictor(all_features, is_train, Y)
...
def train(version, gpu, data_path):
    with tf.Graph().as_default():
        ...
        var_list, update_ops = data_utils.get_vars_and_update_ops('cnn_adversarial')
        cnn_train_op = slim.learning.create_train_op(
            total_loss,
            optimizer,
        ...
        with tf.Session(config=tf_config) as sess:
            ...
            gen_source_only_batch = data_utils.batch_generator(
                source_train, 2*batch_size)
            gen_target_only_batch = data_utils.batch_generator(
            ...
            for i in range(num_steps):
                ...
                _, batch_loss, dloss, lloss, d_acc, l_acc, summary = \
                    sess.run([cnn_train_op, total_loss, domain_loss, label_loss, domain_acc, label_acc, merged],
                ...
                if i%checkpoint_step ==0 or i==num_steps-1:
                    ...
                    print("...Step {}".format(i))
                    print(" ...cnn_adversarial test: source test {}, target test {}".format(source_test_acc,
                                                                                        target_test_acc))

                if i % summary_step == 0 or i == num_steps - 1:
                    print("...Step {}".format(i))
                    print ' ...cnn_adversarial train loss: %f domain_loss %f, label_loss %f, domain_acc: %f  label_acc: %f' % \
                          (batch_loss, dloss, lloss, d_acc, l_acc)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="cnn_adversarial domain adaptation")
    ...
    Realworld_ideal_dislocation_path = os.path.join(datasets_path, 'RealWorld', 'cross_placement_ideal_dislocation.cpkl')

    train(args.version, args.gpu, data_path=Realworld_thigh_shin_path)
